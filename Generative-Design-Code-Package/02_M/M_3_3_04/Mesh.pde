...
class Mesh {
  ...
  float getMeshDistorsion() {
    return meshDistorsion;
  }
  void setMeshDistorsion(float theValue) {
    meshDistorsion = theValue;
  }
    ...
    for (int iv = 0; iv <= ivMax; iv++) {
      if (drawMode == TRIANGLES) {
        ...
        for (int iu = 0; iu <= iuMax; iu++) {
          ...
          beginShape(drawMode);
          float r1 = meshDistorsion * random(-1, 1);
          float r2 = meshDistorsion * random(-1, 1);
          float r3 = meshDistorsion * random(-1, 1);
          ...
          fill(random(minH, maxH), random(minS, maxS), random(minB, maxB), meshAlpha);
          beginShape(drawMode);
          r1 = meshDistorsion * random(-1, 1);
          r2 = meshDistorsion * random(-1, 1);
          r3 = meshDistorsion * random(-1, 1);
          vertex(points[iv+1][iu+1].x+r1, points[iv+1][iu+1].y+r2, points[iv+1][iu+1].z+r3);
          vertex(points[iv][iu].x+r1, points[iv][iu].y+r2, points[iv][iu].z+r3);
